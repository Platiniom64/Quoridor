...
-- ! this works in this file, in order to make the tests pass
-- the higher the better for the current player
utility :: Game -> Int 
utility (Game b ps) = - (distWinning b [currentCell (currentPlayer ps)] (winningPositions (currentPlayer ps)) [currentCell (currentPlayer ps)]) + ((distWinning b [currentCell (previousPlayer ps)] (winningPositions (previousPlayer ps)) [currentCell (previousPlayer ps)]) `div` 2)
...
-- ! version of minimax that uses the utility function "the higher the better", hence it must take the minimum at each level
...
decideActionTOP branches = fst (foldr minim (Move (('a', 1), ('b', 1)), 99999) [ (a, decideValueBOT tree)  | (a, tree) <- branches])
                                where
                                    minim (a, b) (c, d)
                                        | b < d = (a, b)
                                        | otherwise = (c, d)

decideValueBOT :: EvalTree -> Int
decideValueBOT (StateTree v []) = v
decideValueBOT (StateTree v branches) = minimum [ decideValueBOT tree | (a, tree) <- branches]
...
-- ! the implementation of AB pruning does not work in this file (no acutal efficiency optimisation), don't use it here !
...
depth :: Int 
depth = 1 --intially set to 4

-- Given breadth for pruning.
breadth :: Int 
breadth = 20 --intially set to 10
...
minimax =
    ...
    . lowFirst  -- as we want the lowest utility for the opponent
    . evalTree
    . pruneDepth depth
    . generateGameTree 
