...
startingPlayersMiddle ctr1 ctr2 ctr3 ctr4 = 
    [ctr1 "W" (firstColumn, middleRow) wallsPerPlayer winningW,
     ctr2 "X" (middleColumn, firstRow) wallsPerPlayer winningX, 
     ctr3 "Y" (middleColumn, lastRow) wallsPerPlayer winningY,
     ctr4 "Z" (lastColumn, middleRow) wallsPerPlayer winningZ]
    where
        firstColumn = intToColumn 1
        lastColumn = intToColumn (boardSize)
        middleColumn = intToColumn ((div boardSize 2) + 1)
        middleRow = ((div boardSize 2) + 1)
        winningW = [((lastColumn), i)| i<-allRows]
        winningZ = [((firstColumn), i)| i<-allRows]
...
main = do {
    ...
    case (nameToPlayerConstructor playerW, nameToPlayerConstructor playerX, nameToPlayerConstructor playerY, nameToPlayerConstructor playerZ) of 
        (Just ctrW, Just ctrX, Just ctrY, Just ctrZ) -> do { play (Game startingBoard (startingPlayersMiddle ctrW ctrX ctrY ctrZ)) }
