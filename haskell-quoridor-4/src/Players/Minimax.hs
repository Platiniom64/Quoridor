...
-- ! lower the better for the previous player
utility :: Game -> Int 
utility (Game b ps) = sum [ - (distWinning b [currentCell p] (winningPositions p) [currentCell p]) | p <- ps, not (p `theSamePlayer` (previousPlayer ps)) ] + ((distWinning b [currentCell (previousPlayer ps)] (winningPositions (previousPlayer ps)) [currentCell (previousPlayer ps)]) * 3)
            where
...
-- ! modified minimax algorithm, same as the one for the 'basic' file
...
-- ! doesn't work for this 4 player game due to utility used
